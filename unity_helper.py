...
class Il2cpp:
    ...
    # def get_object_from_class(self, assembly_name: str, namespace: str, klass: str):
    #     cls_ptr = self.get_class_from_name(assembly_name, namespace, klass)
    #     if not cls_ptr:
    #         return None
    #     with self._attached_context():
    #         type_ptr = self.il2cpp_class_get_type(ctypes.c_void_p(cls_ptr))
    #         if not type_ptr:
    #             return None

    #         type_obj = self.il2cpp_type_get_object(ctypes.c_void_p(type_ptr))
    #         if not type_obj:
    #             return None
    ...
    # def get_components(self, gameobject_ptr: int, assembly: str, namespace: str, klass: str):
    #     """
    #     Returns an array of components of the given class attached to a GameObject.
    #     Equivalent to: gameObject.GetComponents(typeof(Klass))
    #     """

    #     # Step 1: Get the Class* for the target component type
    #     cls = self.get_class_from_name(assembly, namespace, klass)
    #     if not cls:
    #         raise ValueError(f"Could not find type {namespace}.{klass}")

    #     # Step 2: Convert Class* => Il2CppType* => System.Type object
    #     type_ptr = self.il2cpp_class_get_type(cls)
    #     if not type_ptr:
    #         raise RuntimeError("il2cpp_class_get_type returned null")

    #     system_type_obj = self.il2cpp_type_get_object(type_ptr)
    #     if not system_type_obj:
    #         raise RuntimeError("il2cpp_type_get_object returned null")

    #     # Step 3: Find GameObject.GetComponents(Type)
    ...
    #     if not method:
    #         raise ValueError("GameObject.GetComponents(Type) not found")

    #     # Step 4: Invoke method
    #     arr = self.invoke_method(method, gameobject_ptr, [system_type_obj])

    #     return arr  # This is a managed Il2CppArray* (GameObject components array)
...
# class Mono:
#     def __init__(self, dll_name: str = "Mono-2.0-bdwgc.dll"):
#         self.game_asm = ctypes.WinDLL(dll_name)
#         self.PROCESS = Process()

#         self.mono_domain_get = self.game_asm.mono_domain_get
#         self.mono_domain_get.restype = ctypes.c_void_p
#         self.mono_domain_get.argtypes = []

#         self.mono_thread_attach = self.game_asm.mono_thread_attach
#         self.mono_thread_attach.argtypes = [ctypes.c_void_p]
#         self.mono_thread_attach.restype = ctypes.c_void_p

#         try:
#             self.mono_thread_current = self.game_asm.mono_thread_current
#             self.mono_thread_current.argtypes = []
#             self.mono_thread_current.restype = ctypes.c_void_p
#         except AttributeError:
#             self.mono_thread_current = None

#         try:
#             self.mono_thread_detach = self.game_asm.mono_thread_detach
#             self.mono_thread_detach.argtypes = [ctypes.c_void_p]
#             self.mono_thread_detach.restype = None
#         except AttributeError:
#             self.mono_thread_detach = None

#         self.mono_domain_assembly_open = self.game_asm.mono_domain_assembly_open
#         self.mono_domain_assembly_open.argtypes = [ctypes.c_void_p, ctypes.c_char_p]
#         self.mono_domain_assembly_open.restype = ctypes.c_void_p

#         self.mono_assembly_get_image = self.game_asm.mono_assembly_get_image
#         self.mono_assembly_get_image.argtypes = [ctypes.c_void_p]
#         self.mono_assembly_get_image.restype = ctypes.c_void_p

#         self.mono_class_from_name = self.game_asm.mono_class_from_name
#         self.mono_class_from_name.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.c_char_p]
#         self.mono_class_from_name.restype = ctypes.c_void_p

#         self.mono_class_get_methods = self.game_asm.mono_class_get_methods
#         self.mono_class_get_methods.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)]
#         self.mono_class_get_methods.restype = ctypes.c_void_p

#         self.mono_class_get_method_from_name = self.game_asm.mono_class_get_method_from_name
#         self.mono_class_get_method_from_name.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.c_int]
#         self.mono_class_get_method_from_name.restype = ctypes.c_void_p

#         self.mono_method_get_name = self.game_asm.mono_method_get_name
#         self.mono_method_get_name.argtypes = [ctypes.c_void_p]
#         self.mono_method_get_name.restype = ctypes.c_char_p

#         self.mono_method_signature = self.game_asm.mono_method_signature
#         self.mono_method_signature.argtypes = [ctypes.c_void_p]
#         self.mono_method_signature.restype = ctypes.c_void_p

#         self.mono_signature_get_param_count = self.game_asm.mono_signature_get_param_count
#         self.mono_signature_get_param_count.argtypes = [ctypes.c_void_p]
#         self.mono_signature_get_param_count.restype = ctypes.c_int

#         self.mono_signature_get_params = self.game_asm.mono_signature_get_params
#         self.mono_signature_get_params.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)]
#         self.mono_signature_get_params.restype = ctypes.c_void_p

#         self.mono_signature_get_return_type = self.game_asm.mono_signature_get_return_type
#         self.mono_signature_get_return_type.argtypes = [ctypes.c_void_p]
#         self.mono_signature_get_return_type.restype = ctypes.c_void_p

#         self.mono_runtime_invoke = self.game_asm.mono_runtime_invoke
#         self.mono_runtime_invoke.argtypes = [
#             ctypes.c_void_p,
#             ctypes.c_void_p,
#             ctypes.POINTER(ctypes.c_void_p),
#             ctypes.POINTER(ctypes.c_void_p)
#         ]
#         self.mono_runtime_invoke.restype = ctypes.c_void_p

#         self.mono_class_get_fields = self.game_asm.mono_class_get_fields
#         self.mono_class_get_fields.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)]
#         self.mono_class_get_fields.restype = ctypes.c_void_p

#         self.mono_field_get_name = self.game_asm.mono_field_get_name
#         self.mono_field_get_name.argtypes = [ctypes.c_void_p]
#         self.mono_field_get_name.restype = ctypes.c_char_p

#         self.mono_field_get_type = self.game_asm.mono_field_get_type
#         self.mono_field_get_type.argtypes = [ctypes.c_void_p]
#         self.mono_field_get_type.restype = ctypes.c_void_p

#         self.mono_type_get_name = self.game_asm.mono_type_get_name
#         self.mono_type_get_name.argtypes = [ctypes.c_void_p]
#         self.mono_type_get_name.restype = ctypes.c_char_p

#         self.mono_field_get_offset = self.game_asm.mono_field_get_offset
#         self.mono_field_get_offset.argtypes = [ctypes.c_void_p]
#         self.mono_field_get_offset.restype = ctypes.c_int

#         self.mono_field_get_value = self.game_asm.mono_field_get_value
#         self.mono_field_get_value.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]

#         self.mono_image_get_class = self.game_asm.mono_image_get_class
#         self.mono_image_get_class.argtypes = [ctypes.c_void_p, ctypes.c_int]
#         self.mono_image_get_class.restype = ctypes.c_void_p

#         self.mono_image_get_class_count = self.game_asm.mono_image_get_class_count
#         self.mono_image_get_class_count.argtypes = [ctypes.c_void_p]
#         self.mono_image_get_class_count.restype = ctypes.c_int

#         self.mono_class_get_namespace = self.game_asm.mono_class_get_namespace
#         self.mono_class_get_namespace.argtypes = [ctypes.c_void_p]
#         self.mono_class_get_namespace.restype = ctypes.c_char_p

#         self.mono_class_get_name = self.game_asm.mono_class_get_name
#         self.mono_class_get_name.argtypes = [ctypes.c_void_p]
#         self.mono_class_get_name.restype = ctypes.c_char_p

#         self.mono_field_static_get_value = self.game_asm.mono_field_static_get_value
#         self.mono_field_static_get_value.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
#         self.mono_field_static_get_value.restype = None

#         self.mono_field_get_flags = self.game_asm.mono_field_get_flags
#         self.mono_field_get_flags.argtypes = [ctypes.c_void_p]
#         self.mono_field_get_flags.restype = ctypes.c_int

#         self.mono_field_set_value = self.game_asm.mono_field_set_value
#         self.mono_field_set_value.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
#         self.mono_field_set_value.restype = None

#         self.mono_object_unbox = self.game_asm.mono_object_unbox
#         self.mono_object_unbox.argtypes = [ctypes.c_void_p]
#         self.mono_object_unbox.restype = ctypes.c_void_p

#         self.mono_type_get_object = self.game_asm.mono_type_get_object
#         self.mono_type_get_object.argtypes = [ctypes.c_void_p]
#         self.mono_type_get_object.restype = ctypes.c_void_p

#         self.mono_class_get_type = self.game_asm.mono_class_get_type
#         self.mono_class_get_type.argtypes = [ctypes.c_void_p]
#         self.mono_class_get_type.restype = ctypes.c_void_p

#         self.mono_object_get_class = self.game_asm.mono_object_get_class
#         self.mono_object_get_class.argtypes = [ctypes.c_void_p]
#         self.mono_object_get_class.restype = ctypes.c_void_p

#         self.mono_string_new = self.game_asm.mono_string_new
#         self.mono_string_new.argtypes = [ctypes.c_char_p]
#         self.mono_string_new.restype = ctypes.c_void_p

#         self.mono_method_get_flags = self.game_asm.mono_method_get_flags
#         self.mono_method_get_flags.argtypes = [ctypes.c_void_p, ctypes.c_int]
#         self.mono_method_get_flags.restype = ctypes.c_int
...
#             dom = self._get_domain_raw()
#         if not dom:
#             raise RuntimeError("mono domain not available")
#         self._domain = dom
#         self.mono_thread_attach(ctypes.c_void_p(dom))
#         self._attached = True
#         return dom
...
#         current = None
#         try:
#             current = self.mono_thread_current()
#         except Exception:
#             current = None
#         did_attach = False
#         thread = None
#         if not current:
#             thread = self.mono_thread_attach(ctypes.c_void_p(dom))
#             did_attach = True
#         try:
#             yield
#         finally:
#             if did_attach:
#                 try:
#                     self.mono_thread_detach(thread)
#                 except Exception:
#                     pass
...
#                 return self._assembly_cache[assembly_name]
#             dom = self._get_domain_raw()
#             asm = self.mono_domain_assembly_open(ctypes.c_void_p(dom), assembly_name.encode())
#             if not asm:
#                 return None
#             self._assembly_cache[assembly_name] = int(asm)
#             return int(asm)
...
#             return None
#         with self._attached_context():
#             cls = self.mono_class_from_name(ctypes.c_void_p(img), namespace.encode(), klass.encode())
#             if not cls:
#                 return None
#             self._class_cache[key] = int(cls)
#             return int(cls)
...
#         for m in methods:
#             if m == method_name:
#                 return int(methods[m]["method_ptr"])
#         return None
...
#                     cv = ctypes.c_void_p(0)
#                 elif isinstance(v, str):
#                     cv = ctypes.c_void_p(self.mono_string_new(ctypes.c_void_p(self._domain), v.encode()))
#                 else:
#                     cv = ctypes.c_void_p(int(v))
#                 local_refs.append(cv)
#                 args[i] = ctypes.cast(ctypes.pointer(cv), ctypes.c_void_p)
#             exc = ctypes.c_void_p()
#             ret = self.mono_runtime_invoke(
#                 ctypes.c_void_p(method_ptr),
#                 ctypes.c_void_p(instance) if instance else None,
#                 args if argc else None,
#                 ctypes.byref(exc)
...
#             if not ret:
#                 return None
#             unboxed = self.mono_object_unbox(ret)
#             return unboxed if unboxed else int(ret)
...
#     def list_fields_in_class(self, assembly_name: str, namespace: str, klass: str) -> Dict[str, Any]:
#         cls_ptr = self.get_class_from_name(assembly_name, namespace, klass)
#         if not cls_ptr:
#             return {}
#         fields = {}
...
#         with self._attached_context():
#             while True:
#                 field = self.mono_class_get_fields(ctypes.c_void_p(cls_ptr), ctypes.byref(iterator))
#                 if not field:
#                     break
#                 name_ptr = self.mono_field_get_name(field)
#                 name = name_ptr.decode() if name_ptr else ""
#                 type_ptr = self.mono_field_get_type(field)
#                 type_name = self.mono_type_get_name(type_ptr).decode() if type_ptr else ""
#                 is_static = (self.mono_field_get_flags(field) & 0x0010) != 0
#                 fields[name] = {"field_ptr": int(field), "type": type_name, "static": is_static}
#         return fields


#     def list_static_fields_in_class(self, assembly_name: str, namespace: str, klass: str) -> List[str]:
#         fields = self.list_fields_in_class(assembly_name, namespace, klass)
#         return [f for f in fields if fields[f]["static"]]


#     def find_field_in_class(self, assembly_name: str, namespace: str, klass: str, field_name: str) -> Optional[int]:
#         fields = self.list_fields_in_class(assembly_name, namespace, klass)
#         if field_name in fields:
#             return fields[field_name]["field_ptr"]
#         return None
...
#         with self._attached_context():
#             buf = ctypes.c_void_p()
#             self.mono_field_get_value(ctypes.c_void_p(instance), ctypes.c_void_p(field_ptr), ctypes.byref(buf))
#             type_ptr = self.mono_field_get_type(ctypes.c_void_p(field_ptr))
#             type_name = self.mono_type_get_name(type_ptr).decode() if type_ptr else ""
#         val = buf.value
#         if "Single" in type_name:
#             return ctypes.cast(ctypes.byref(ctypes.c_void_p(val)), ctypes.POINTER(ctypes.c_float))[0]
#         if "Double" in type_name:
#             return ctypes.cast(ctypes.byref(ctypes.c_void_p(val)), ctypes.POINTER(ctypes.c_double))[0]
#         if "Int32" in type_name:
#             return ctypes.cast(ctypes.byref(ctypes.c_void_p(val)), ctypes.POINTER(ctypes.c_int))[0]
#         if "Int64" in type_name:
#             return ctypes.cast(ctypes.byref(ctypes.c_void_p(val)), ctypes.POINTER(ctypes.c_longlong))[0]
...
#             else:
#                 cval = ctypes.c_void_p(int(value))
#             self.mono_field_set_value(ctypes.c_void_p(instance), ctypes.c_void_p(field_ptr), ctypes.byref(cval))
#         return True
...
#         with self._attached_context():
#             buf = ctypes.c_void_p()
#             self.mono_field_static_get_value(ctypes.c_void_p(field_ptr), ctypes.byref(buf))
#         return buf.value


#     def list_methods_in_class(self, assembly_name: str, namespace: str, klass: str, cache: bool = True) -> Dict[str, Any]:
#         key = f"{assembly_name}|{namespace}|{klass}|methods"
#         if cache and key in self._class_cache:
#             return self._class_cache[key]
#         cls_ptr = self.get_class_from_name(assembly_name, namespace, klass)
...
#         with self._attached_context():
#             while True:
#                 method = self.mono_class_get_methods(ctypes.c_void_p(cls_ptr), ctypes.byref(iterator))
#                 if not method:
#                     break
#                 name_ptr = self.mono_method_get_name(method)
#                 name = name_ptr.decode() if name_ptr else ""
#                 sig = self.mono_method_signature(method)
#                 param_count = self.mono_signature_get_param_count(sig)
#                 ret_type = self.mono_signature_get_return_type(method)
#                 ret_name = self.mono_type_get_name(ret_type).decode() if ret_type else ""
#                 flags = self.mono_method_get_flags(method, None)
#                 methods[name] = {
#                     "method_ptr": int(method),
#                     "method_address": int(method),
#                     "param_count": param_count,
#                     "param_info": [],
#                     "return_value": ret_name,
#                     "flags": {"static": (flags & 0x0010) != 0}
#                 }
#         if cache:
#             self._class_cache[key] = methods
#         return methods
...
#         for klass in classes:
#             data.setdefault(klass, {}).setdefault("fields", [])
#             for f in self.list_static_fields_in_class(assembly_name, "", klass):
#                 data[klass]["fields"].append(f)
#         return data
...
#         classes = []
#         with self._attached_context():
#             count = self.mono_image_get_class_count(ctypes.c_void_p(img), 2)
#             for i in range(count):
#                 cls = self.mono_image_get_class(ctypes.c_void_p(img), i)
#                 if not cls:
#                     continue
#                 ns = self.mono_class_get_namespace(ctypes.c_void_p(cls))
#                 ns_name = ns.decode() if ns else ""
#                 name = self.mono_class_get_name(ctypes.c_void_p(cls))
#                 cls_name = name.decode() if name else ""
#                 classes.append(f"{ns_name}.{cls_name}" if ns_name else cls_name)
#                 key = f"{assembly_name}|{ns_name}|{cls_name}"
#                 self._class_cache[key] = int(cls)
#         return classes
    ...
    #         return None
    #     with self._attached_context():
    #         type_ptr = self.mono_class_get_type(ctypes.c_void_p(cls_ptr))
    #         if not type_ptr:
    #             return None

    #         type_obj = self.mono_type_get_object(ctypes.c_void_p(type_ptr))
    #         if not type_obj:
    #             return None
    ...
    #     # Step 2: Convert Class* => Il2CppType* => System.Type object
    #     type_ptr = self.mono_class_get_type(cls)
    #     if not type_ptr:
    #         raise RuntimeError("mono_class_get_type returned null")

    #     system_type_obj = self.mono_type_get_object(type_ptr)
    #     if not system_type_obj:
